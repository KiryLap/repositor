...
private:
    ...
    double calculateMaxFlow(const Connection& conn);

public:
    int inputDiameter();
    void connectStations(map<int, Pipe>& pipes, map<int, CompressorStation>& stations, PipeManager& pipeManager);
    ...
    string saveConnections() const;
    void findShortestPath(const map<int, Connection>& connections);
